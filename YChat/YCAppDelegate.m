...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    _XMPP_RESOURCE_NAME = [[NSString stringWithFormat:@"YookosMessenger-%@-%@",[[UIDevice currentDevice] localizedModel],[[[UIDevice currentDevice] identifierForVendor] UUIDString]] stringByReplacingOccurrencesOfString:@" " withString:@""];
...
{
	DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    if ([YAppDelegate.chatConnectionDelegate respondsToSelector:@selector(loginSuccessful)]) {
        [YAppDelegate.chatConnectionDelegate loginSuccessful];
    }
}
...
{
	DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    if ([YAppDelegate.chatConnectionDelegate respondsToSelector:@selector(loginUnsuccessful)]) {
        [YAppDelegate.chatConnectionDelegate loginUnsuccessful];
    }
}
                    ...
                    withDictionary:(NSDictionary*)dictionary andCallback:(requestCompletedBlock)completionResponse
{
    ...
    NSError *error;
    if([YAppDelegate.managedObjectContext save:&error])
    {
...
- (void)clearObjectForEntityName:(NSString*)entityName withPredicate:(NSPredicate*)predicate inManagedObjectContext:(NSManagedObjectContext*)context andCallback:(requestCompletedBlock)completionResponse
{
    ...
    NSError *error = nil;
    NSArray *allObjects = [YAppDelegate.managedObjectContext executeFetchRequest:fetchObject error:&error];
    ...
    if (![YAppDelegate.managedObjectContext save:&saveError]) {
